"""Plane fill vs trained structural UNet — job017 desk removal (scene0000_00)."""
import json, re
...
COMP  = SCENE / "completion/scene0000_00_job017_plane_fill_no_nnwall"
OUT   = HYPO / "figs"

plt.rcParams.update({"font.family": "serif", "font.size": 10,
                     "axes.titlesize": 11, "figure.dpi": 150,
                     "savefig.dpi": 300, "savefig.bbox": "tight"})
VOID_COL = "#ff7f0e"; PL_COL = "#9C27B0"; ST_COL = "#2ca02c"

def load_ply(path):
    data = Path(path).read_bytes()
    hi = data.find(b"end_header"); nl = data[hi + len("end_header")]
...
def box(ax, c, s, proj, col=VOID_COL, lw=1.5, ls="--"):
    ...
    ax.add_patch(plt.Rectangle(
        ...
        fill=False, edgecolor=col, linewidth=lw, linestyle=ls, zorder=5))
...
# ── Load data ──────────────────────────────────────────────────────────────────
meta   = json.loads((COMP / "metadata.json").read_text())
center = np.array(meta["remove_center"])
size   = np.array(meta["remove_size"])

bef, bef_rgb = load_ply(COMP / "crop_before_remove.ply")
rem, rem_rgb = load_ply(COMP / "crop_after_remove.ply")
pl,  _       = load_ply(COMP / "nn_floor_fill_completed_points.ply")
st,  _       = load_ply(COMP / "structural_surface_completed_points.ply")
print(f"before={bef.shape[0]}  void={rem.shape[0]}  plane={pl.shape[0]}  struct={st.shape[0]}")

PAD = 2.6
bef_c, bef_c_rgb = crop(bef, bef_rgb, center, size, PAD)
rem_c, rem_c_rgb = crop(rem, rem_rgb, center, size, PAD)
pl_c,  _         = crop(pl,  None,    center, size, PAD)
st_c,  _         = crop(st,  None,    center, size, PAD)

cx, cy, cz = center; sx, sy, sz = size
PADL = 0.5
xlim = (cx - sx / 2 - PADL, cx + sx / 2 + PADL)
ylim = (cy - sy / 2 - PADL, cy + sy / 2 + PADL)
zlim = (cz - sz / 2 - PADL, cz + sz / 2 + PADL)
...
titles = [
    "(a) Before removal\n(original scene)",
    "(b) After removal\n(desk void exposed)",
    f"(c) Planar surface fill\n(RANSAC-fitted planes, {pl.shape[0]} pts)",
    f"(d) Trained structural UNet\n(learned surfaces, {st.shape[0]} pts)",
]

# ── Plot ───────────────────────────────────────────────────────────────────────
fig = plt.figure(figsize=(15, 7.5), facecolor="white")
gs  = GridSpec(2, 4, figure=fig, hspace=0.38, wspace=0.22,
...
for r, proj in enumerate(PROJS):
    ...
    for c in range(4):
        ...
        if c == 0:
            sc(ax, bef_c, bef_c_rgb, proj=proj, alpha=0.65)
            box(ax, center, size, proj, VOID_COL, 1.6)
        elif c == 1:
            sc(ax, rem_c, rem_c_rgb, proj=proj, alpha=0.55)
            box(ax, center, size, proj, VOID_COL, 1.6)
        elif c == 2:
            sc(ax, rem_c, rem_c_rgb, proj=proj, alpha=0.30)
            sc(ax, pl_c, color=PL_COL, proj=proj, alpha=0.8, ms=5)
            box(ax, center, size, proj, "#888", 1.2)
        else:
            sc(ax, rem_c, rem_c_rgb, proj=proj, alpha=0.30)
            sc(ax, st_c, color=ST_COL, proj=proj, alpha=0.85, ms=5)
            box(ax, center, size, proj, "#888", 1.2)
...
for r, txt in enumerate(["Top-down view", "Front view"]):
    ...
    fig.text(0.005, p.y0 + p.height / 2, txt, va="center", ha="left",
             fontsize=9, color="#444", rotation=90, fontweight="bold")

handles = [
    mpatches.Patch(color="#aec7e8", label="Scene geometry (RGB)"),
    mpatches.Patch(color=PL_COL,    label="Planar surface fill (RANSAC)"),
...
fig.legend(handles=handles, loc="lower center", ncol=4, framealpha=0.9,
           edgecolor="#ccc", fontsize=8.5, bbox_to_anchor=(0.5, 0.01))
fig.suptitle(
    "Background Fill: Planar Surface Fill vs Trained Structural Model"
    " — scene0000\\_00 / job017 (desk removal)",
    fontsize=12, color="#111", y=0.95, fontweight="bold")

for ext in ("png", "pdf"):
    plt.savefig(OUT / f"fig_nn_vs_trained_desk.{ext}", facecolor="white")
    print("Saved", OUT / f"fig_nn_vs_trained_desk.{ext}")
plt.close()
