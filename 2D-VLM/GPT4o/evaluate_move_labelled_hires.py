...
EVAL_JSON  = HYPO_ROOT / "dataset" / "contextvqa_move_eval_labelled_hi.json"
REQUESTS_DIR   = HYPO_ROOT / "dataset" / "move_eval_gpt4o_requests_labelled_hires"
...
PROMPT = """You are given a top-down (bird's-eye) view of a 3D indoor scene after an object has been moved.
...
Use the scene orientation above to determine spatial directions (front/back/left/right) when answering.
...
MODEL = "gpt-4o"
REQUESTS_PER_FILE = 15   # 15 reqs × ~1,100 tokens ≈ 16k tokens, well under 90k org limit
AXIS_DEF = HYPO_ROOT / "dataset" / "axis_definition.xlsx"
ORIENT_CONTEXTS = HYPO_ROOT / "dataset" / "orientation_contexts.json"
...
OUT_JSON   = HYPO_ROOT / "dataset" / "contextvqa_move_eval_gpt4o_labelled_hires.json"
BATCH_IDS_FILE = HYPO_ROOT / "dataset" / "move_eval_gpt4o_batch_ids_labelled_hires.json"
REQUESTS_DIR   = HYPO_ROOT / "dataset" / "move_eval_gpt4o_requests_labelled_hires"


def load_orientation_map() -> dict[str, str]:
    """Return {scene_id: rich_orientation_string} from pre-computed orientation_contexts.json."""
    if ORIENT_CONTEXTS.exists():
        return json.loads(ORIENT_CONTEXTS.read_text())
    # Fallback to simple landmark hints if pre-computed file not found
...
def encode_image(path: Path, size: int = 1024) -> str:
...
def submit_and_collect(client: OpenAI, jsonl_files: list[Path]) -> dict[str, str]:
    ...
    for i, fpath in enumerate(jsonl_files, 1):
        ...
        for attempt in range(1, 11):
            ...
            # Give OpenAI time to validate and surface token_limit errors (~60-90s)
            time.sleep(90)
