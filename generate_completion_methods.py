"""Three background-completion methods on job015 cabinet: ConvONet vs NN vs trained structural."""
import json, re
...
VOID_COL="#ff7f0e";CONV_COL="#d62728";NN_COL="#9C27B0";ST_COL="#2ca02c"
...
def sc(ax,xyz,rgb=None,color=None,alpha=0.7,proj=(0,2),ms=None):
    ...
    n=xyz.shape[0];s=ms if ms else (3 if n<20000 else 1.0)
...
def box(ax,c,s,proj,col=VOID_COL,lw=1.4,ls="--"):
...
def style(ax,xl,yl):
    ax.set_facecolor("white");ax.set_xlabel(xl,fontsize=7.5,color="#555");ax.set_ylabel(yl,fontsize=7.5,color="#555")
    ax.tick_params(labelsize=6.5,color="#aaa",labelcolor="#666");ax.grid(True,color="#eee",lw=0.5,zorder=0)
...
rem,rem_rgb=load_ply(HYPO/mani["canonical_edit_file"])
conv,_=load_ply(COMP/"convonet_official_points.ply")
nn,_=load_ply(COMP/"nn_floor_fill_completed_points.ply")
st,_=load_ply(COMP/"structural_surface_completed_points.ply")
print(f"void={rem.shape[0]} conv={conv.shape[0]} nn={nn.shape[0]} st={st.shape[0]}")

PAD=2.4
rem_c,rem_c_rgb=crop(rem,rem_rgb,center,size,PAD)
conv_c,_=crop(conv,None,center,size,PAD)
nn_c,_=crop(nn,None,center,size,PAD)
st_c,_=crop(st,None,center,size,PAD)
...
titles=["(a) After removal\n(void)",
        "(b) ConvONet\n(official, rejected)",
        "(c) NN floor fill\n(non-learned)",
        "(d) Trained structural\n(used)"]
overlays=[(None,None),(conv_c,CONV_COL),(nn_c,NN_COL),(st_c,ST_COL)]

fig=plt.figure(figsize=(15,7.5),facecolor="white")
gs=GridSpec(2,4,figure=fig,hspace=0.34,wspace=0.24,top=0.87,bottom=0.10,left=0.05,right=0.98)
for r,proj in enumerate(PROJS):
    lx,ly=LIMS[r]
    for c in range(4):
        ax=fig.add_subplot(gs[r,c]);style(ax,RL[r][0],RL[r][1])
        if r==0:ax.set_title(titles[c],fontsize=9.5,pad=6,color="#222",fontweight="bold")
        if c==0:
            sc(ax,rem_c,rem_c_rgb,proj=proj,alpha=0.55);box(ax,center,size,proj,VOID_COL,1.6)
        else:
            sc(ax,rem_c,rem_c_rgb,proj=proj,alpha=0.28)
            ov,col=overlays[c];sc(ax,ov,color=col,proj=proj,alpha=0.55 if c==1 else 0.8,ms=1.0 if c==1 else 4)
            box(ax,center,size,proj,"#888",1.1)
        ax.set_xlim(*lx);ax.set_ylim(*ly);ax.set_aspect("equal")
for r,txt in enumerate(["Top-down view","Front view"]):
    p=fig.get_axes()[r*4].get_position()
    fig.text(0.004,p.y0+p.height/2,txt,va="center",ha="left",fontsize=9,color="#444",rotation=90,fontweight="bold")
handles=[mpatches.Patch(color="#aec7e8",label="Scene (RGB)"),
         mpatches.Patch(color=CONV_COL,label="ConvONet (over-fills, rejected)"),
         mpatches.Patch(color=NN_COL,label="NN floor fill (floor only)"),
         mpatches.Patch(color=ST_COL,label="Trained structural UNet (used)"),
         mpatches.Patch(facecolor="none",edgecolor=VOID_COL,linestyle="--",linewidth=1.4,label="Removal box")]
fig.legend(handles=handles,loc="lower center",ncol=5,framealpha=0.9,edgecolor="#ccc",fontsize=8.5,bbox_to_anchor=(0.5,0.01))
fig.suptitle("Background Completion Methods — scene0000\\_00 / job015 (cabinet): ConvONet vs NN vs Trained Structural",
             fontsize=12,color="#111",y=0.94,fontweight="bold")
for ext in("png","pdf"):
    plt.savefig(OUT/f"fig_completion_methods.{ext}",facecolor="white");print("Saved",OUT/f"fig_completion_methods.{ext}")
plt.close()
