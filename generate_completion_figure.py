"""
Object completion figure — coffee table (inst 35, scene0000_00).

Layout: 3 columns × 2 rows
  Col 0  Scene context   — full scene, coffee table highlighted
  Col 1  Partial input   — isolated partial scan cloud (792 pts)
  Col 2  PBIC completion — completed dense shape (2048 pts) + partial (grey)

  Row 0  Top-down  (X – Z)
  Row 1  Front     (X – Y)

Output: figs/fig_completion.pdf / .png
"""
...
COMP_DIR  = PROC / "completion/pbic_inst035_coffee table"
OUT_DIR   = HYPO / "figs"
...
# ── PLY loader (handles both ASCII and binary) ────────────────────────────────
def load_ply(path: Path):
    ...
    # skip \r if Windows line endings
    if data[he - 1] != ord('\n'):
        he += 1
...
# Colour-transfer: assign each predicted point the colour of its nearest partial neighbour
from scipy.spatial import cKDTree
_, nn_idx = cKDTree(partial_xyz).query(completed_xyz, k=1)
completed_rgb = partial_rgb[nn_idx]   # uint8 [0,255]
print(f"  Colour-transferred from {partial_xyz.shape[0]} partial pts")
...
def sc_rgb(ax, xyz, rgb, alpha=0.50, ms=None, proj=(0,2)):
    if xyz is None or xyz.shape[0] == 0: return
    n = xyz.shape[0]
    s = ms if ms else (1.5 if n < 20_000 else 0.5)
    c = rgb.astype(np.float32) / 255.0   # uint8 [0,255] → float [0,1]
...
# for object panels, use combined extent of partial + completed
...
XLBLS  = ["X (m)", "X (m)"]
YLBLS  = ["Z (m)", "Y (m)"]
ROW_NAMES = ["Top-down (X – Z)", "Front (X – Y)"]

COL_TITLES = [
    f"(a) Scene context\n({LABEL.capitalize()} highlighted)",
    "(b) Partial input\n(raw scan extract, 792 pts)",
    f"(c) PBIC completion\n(dense prediction, 2048 pts)",
]
...
scene_sub  = sub(xyz_all[~mask_obj], 40_000)
scene_sub_rgb = rgb_all[~mask_obj]
idx_sub = RNG.choice(xyz_all[~mask_obj].shape[0], min(40_000, xyz_all[~mask_obj].shape[0]), replace=False)
scene_sub_rgb = rgb_all[~mask_obj][idx_sub]

for r, proj in enumerate(PROJS):
    lim_x     = xlim_scene
    lim_scene = zlim_scene if r == 0 else ylim_scene
    lim_obj   = zlim_obj   if r == 0 else ylim_obj
    ...
    for c in range(3):
        ...
        if c == 0:
            # full scene with object highlighted
            style(ax, t, xl, yl, lim_x, lim_scene)
            sc_rgb(ax, scene_sub, scene_sub_rgb, alpha=0.40, proj=proj)
            sc(ax, partial_xyz, C_OBJECT, alpha=0.95, ms=5, proj=proj)
            draw_box(ax, obj_ctr, obj_size + 0.15, proj, C_BOX, lw=1.3)
            if r == 0:
                ax.text(obj_ctr[0], obj_ctr[2] + obj_size[2]/2 + 0.35,
            ...
            # isolated partial input — scan RGB
            style(ax, t, xl, yl, xlim_obj, lim_obj)
            ...
            # PBIC completed shape — both partial and predicted in scan RGB
            style(ax, t, xl, yl, xlim_obj, lim_obj)
            sc_rgb(ax, completed_xyz, completed_rgb, alpha=0.85, ms=3,  proj=proj)
            sc_rgb(ax, partial_xyz,   partial_rgb,   alpha=0.95, ms=8,  proj=proj)
            ax.text(0.05, 0.05,
...
# ── Legend ────────────────────────────────────────────────────────────────────
handles = [
    mpatches.Patch(color=C_SCENE,   label="Scene geometry (RGB)"),
    mpatches.Patch(color="#A0522D", label="Partial scan input (RGB)"),
    mpatches.Patch(color="#A0522D", label="PBIC completion (colour-transferred from partial)"),
]
...
fig.suptitle(
    f"Instance Completion — {LABEL.capitalize()} (scene0000\\_00, inst {INST_ID})\n"
    f"Partial: {partial_xyz.shape[0]} pts  →  PBIC prediction: {completed_xyz.shape[0]} pts",
    fontsize=11, fontweight="bold", color="#111", y=0.96,
)

for ext in ("pdf", "png"):
    out = OUT_DIR / f"fig_completion.{ext}"
